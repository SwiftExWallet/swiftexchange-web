import { AlertCircle, ChevronRight, Copy, Info, Loader2, RefreshCw, Wallet } from 'lucide-react';
...
// Formats a BigNumber amount, trimming trailing zeros, capped at 8 decimals for very high-precision assets
const formatAmount = (value: BigNumber, decimals: number): string => {
  const capped = decimals > 10 ? 8 : decimals;
  return value
    .toFixed(capped)
    .replace(/(\.[0-9]*[1-9])0+$/, '$1')
    .replace(/\.0+$/, '');
...
interface TransactionReviewProps {
  ...
  currentChainLogo: string | null | undefined;
  needsTrustline: boolean;
  recipientNeedsTrustline: boolean;
  buttonLabel: string;
  onBack: () => void;
  onConfirm: () => void;
}

const TransactionReview = React.memo<TransactionReviewProps>(function TransactionReview({
  currentAsset,
  recipientAddress,
  amount,
  senderAddress,
  ...
  return (
    <div className="space-y-4 w-full overflow-hidden animate-fadeIn">
          ...
          {needsTrustline && currentAsset.type === 'stellar' && (
            <div className="bg-brand-primary/5 border border-brand-primary/20 rounded-xl p-3.5 flex gap-3 items-center">
              ...
              <div>
                ...
                <p className="text-[9px] text-text-secondary font-medium opacity-80 leading-tight mt-0.5">
                  Your account will first add a trustline for {currentAsset.symbol} in this
                  transaction.
                </p>
              </div>
            </div>
            ...
            <div className="bg-bg-primary/50 rounded-xl p-4">
              ...
              <div className="flex justify-between items-center gap-4 min-w-0">
                <span className="text-sm font-bold text-text-primary shrink-0">
                  Total {currentAsset.isNative ? 'Cost' : 'to Send'}
                </span>
                <span
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={onBack} className="btn-secondary btn py-4 rounded-xl font-bold">
          Edit
        </button>
        <TransactionButton label={buttonLabel} onClick={onConfirm} className="font-black" />
      </div>
    </div>
  );
});
...
const TransactionStatus = React.memo<TransactionStatusProps>(function TransactionStatus({ step }) {
  if (step !== 'signing') return null;
  ...
  return (
    <div className="bg-bg-secondary border border-divider rounded-xl text-center py-12 px-6 max-w-sm mx-auto shadow-sm">
      ...
      <p className="text-sm text-text-secondary font-medium px-4">
        Confirm the transaction signature in your wallet extensions.
      </p>
    </div>
  );
});
...
const SendAssets: React.FC<SendCryptoProps> = ({ onBack }) => {
            ...
            <div className="p-4">
              ...
              <div className="bg-bg-secondary/70 rounded-xl px-3 h-14 flex items-center border border-divider/20 focus-within:border-brand-primary/40 transition-colors">
                <input
                  ...
                  type="text"
                  id="recipientAddress"
                  className={`w-full bg-transparent border-none focus:ring-0 outline-none text-xs font-mono placeholder:text-text-muted ${
                    formError && formError.includes('address') ? 'text-danger' : 'text-text-primary'
                  }`}
                  placeholder={
            ...
            <div className="p-4">
              ...
              <div className="bg-bg-secondary/70 rounded-xl px-3 h-14 flex items-center border border-divider/20 focus-within:border-brand-primary/40 transition-colors">
                <input
                  ...
                  inputMode="decimal"
                  id="amount"
                  className={`flex-1 bg-transparent border-none focus:ring-0 outline-none text-lg font-black placeholder:text-text-muted ${
                    formError && (formError.includes('balance') || formError.includes('Amount'))
                      ? 'text-danger'
                      : 'text-text-primary'
                  }`}
                  placeholder="0.00"
  ...
  const renderStep = () => {
    switch (transactionState.step) {
      ...
      case 'review':
        return (
          <TransactionReview
            ...
            currentChainLogo={currentChainLogo}
            needsTrustline={needsTrustline}
            recipientNeedsTrustline={recipientNeedsTrustline}
      ...
      default:
        return <TransactionStatus step={transactionState.step} />;
    }
  };
