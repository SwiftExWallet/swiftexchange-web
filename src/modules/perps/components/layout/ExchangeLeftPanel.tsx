    ...
    <>
      ...
      <div className="flex-1 w-full h-full bg-primary relative">
        <div className="absolute inset-0">
          {activeTab === 'price' && (
            <TradingChart activeChartTab={activeTab} onChartTabChange={setActiveTab} />
          )}
          {activeTab === 'depth' && (
            <DepthChart activeChartTab={activeTab} onChartTabChange={setActiveTab} />
          )}
          {activeTab === 'details' && (
            <DetailsTab activeChartTab={activeTab} onChartTabChange={setActiveTab} />
          )}
        </div>
      </div>
    </>
          ...
          <div className="flex-1 overflow-y-auto flex items-center justify-center h-full">
            <div className="flex flex-col items-center gap-1.5">
              ...
              <span className="text-muted text-[11px]">
                Your {activeTab.toLowerCase()} will show up here
              </span>
            </div>
          </div>
        );
        ...
        <div className="flex items-center gap-5 h-full whitespace-nowrap overflow-x-auto scrollbar-none">
          {tabs.map(tab => (
            <button
              ...
              className={`h-full px-1 flex items-center gap-1.5 transition-colors ${activeTab === tab ? 'text-primary font-medium border-b-2 border-brand' : 'hover:text-primary'}`}
            >
              {tab}
              {tab === 'Open Orders' && (
                <span className="bg-tertiary text-[10px] py-0.5 px-1.5 rounded-full">
                  {
